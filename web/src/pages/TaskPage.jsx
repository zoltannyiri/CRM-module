...
export default function TaskPage() {
  ...
  return (
    <div className="min-h-dvh bg-[#f3f5f6] text-[#253238]">
      <Topbar searchValue={query} onSearchChange={setQuery} onCreate={hasPermission("TASKS_CREATE") ? openCreate : undefined} />
      <div className="flex flex-wrap items-center justify-between gap-3 border-y border-[#e3e8e6] bg-white px-5 py-3 lg:px-7">
        <div className="flex flex-wrap items-center gap-2">
          ...
          {canUseProjects && <select value={projectFilter} onChange={(event) => setProjectFilter(event.target.value)} aria-label="Projektek" className={`${lightControl} min-w-[170px]`}>
        ...
        <div className="flex items-center gap-2">
          ...
          {hasPermission("TASKS_CREATE") && <button type="button" onClick={openCreate} className="inline-flex h-9 cursor-pointer items-center gap-2 rounded-md border border-[#172a2e] bg-[#21343a] px-4 text-xs font-semibold text-white hover:bg-[#17282d]">Új feladat <i className="pi pi-chevron-down text-[10px]" aria-hidden="true" /></button>}
        </div>
      </div>
