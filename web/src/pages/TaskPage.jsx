...
export default function TaskPage() {
  ...
  const openCreate = () => {
    if (!hasPermission("TASKS_CREATE")) {
      showError(
        "Nincs jogosultsága új feladat létrehozásához.",
        "Nincs jogosultság",
      );
      return;
    }
  ...
  const openEdit = (task) => {
    if (!hasPermission("TASKS_EDIT")) {
      showError(
        "Nincs jogosultsága a feladat módosításához.",
        "Nincs jogosultság",
      );
      return;
    }
  ...
  return (
    <div className="min-h-dvh bg-[#f3f5f6] text-[#253238]">
      <Topbar
        ...
        onCreate={hasPermission("TASKS_CREATE") ? openCreate : undefined}
      />
      <div className="flex flex-wrap items-center justify-between gap-3 border-y border-[#e3e8e6] bg-white px-5 py-3 lg:px-7">
        ...
        <div className="flex items-center gap-2">
          ...
          >
            Létrehozás{" "}
            <i
              className={`pi pi-chevron-down text-[10px] transition-transform ${sortDirection === "asc" ? "rotate-180" : ""}`}
              aria-hidden="true"
            />
          </button>
      ...
      <div className="px-5 py-5">
        <TaskListComponent
          ...
          reloadKey={reloadKey}
          onEdit={openEdit}
          canView={hasPermission("TASKS_VIEW")}
