...
export default function OfferPage() {
  ...
  const canUsePartners =
    hasModule("PARTNERS") && hasPermission("PARTNERS_VIEW");
  const canUseProjects =
    hasModule("PROJECTS") && hasPermission("PROJECTS_VIEW");
  ...
  const openCreate = () => {
    if (!canCreate) {
      showError(
        "Ajánlat létrehozásához létrehozási és partner megtekintési jogosultság szükséges.",
        "Nincs jogosultság",
      );
      return;
    }
  ...
  const openEdit = async (offer) => {
    ...
    try {
      const { data } = await apiClient.get(`/offers/${offer.id}`, {
      ...
      showError(error.response?.data?.message || "Az ajánlat nem tölthető be.");
    }
  };
  ...
  if (id) {
    return (
      <div className="min-h-dvh bg-[#f3f5f6] text-[#253238]">
        <Topbar />
        <div className="flex items-center gap-3 border-b border-[#e3e8e6] bg-white px-5 py-3 lg:px-7">
          <button
            type="button"
            onClick={() => navigate("/offer")}
            className="inline-flex h-9 cursor-pointer items-center gap-2 rounded-md border border-[#d6dddc] bg-white px-3 text-xs font-medium"
          >
            <i className="pi pi-arrow-left text-xs" aria-hidden="true" />
            Vissza az ajánlatokhoz
          </button>
          <span className="h-4 w-px bg-[#dbe1df]" />
          <h1 className="text-sm font-semibold">Ajánlat adatlap</h1>
        </div>
        <div className="px-5 py-6 lg:px-7">
          <TabView className={tabsClass}>
    ...
    <div className="min-h-dvh bg-[#f3f5f6] text-[#253238]">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 border-y border-[#e3e8e6] bg-white px-5 py-3 lg:px-7">
        <div className="flex flex-wrap gap-2">
          <select
            ...
            className={`${lightControl} min-w-[145px]`}
            aria-label="Ajánlatstátusz"
          >
        ...
        <div className="flex items-center gap-2">
          <span className="text-xs">Rendezés</span>
          <button
            type="button"
            onClick={() =>
              setSortDirection((value) => (value === "desc" ? "asc" : "desc"))
            }
            className={`${lightControl} inline-flex min-w-[125px] items-center justify-between`}
          >
            Létrehozás{" "}
            <i
              className={`pi pi-chevron-down text-[10px] ${sortDirection === "asc" ? "rotate-180" : ""}`}
              aria-hidden="true"
            />
          </button>
          {canCreate && (
            <button
              type="button"
              onClick={openCreate}
              className="inline-flex h-9 cursor-pointer items-center gap-2 rounded-md border border-[#172a2e] bg-[#21343a] px-4 text-xs font-semibold text-white"
            >
              Új ajánlat{" "}
              <i
                className="pi pi-chevron-down text-[10px]"
                aria-hidden="true"
              />
            </button>
          )}
      ...
      <div className="px-5 py-5">
        <OfferListComponent
          ...
          reloadKey={reloadKey}
          onView={(offer) => navigate(`/offer/${offer.id}`)}
          onEdit={openEdit}
          canView={hasPermission("OFFERS_VIEW")}
          canEdit={hasPermission("OFFERS_EDIT")}
          canDelete={hasPermission("OFFERS_DELETE")}
        />
      </div>
      {formOpen && (
        <OfferFormComponent
