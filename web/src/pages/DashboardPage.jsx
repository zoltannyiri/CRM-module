...
import { browserTimeZone, formatFollowUpTime, followUpTypeLabels } from "../components/followUp/followUpDisplay.js";
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    let active = true;
    apiClient.get("/dashboard", { params: { timeZone: browserTimeZone() } }).then(({ data }) => { if (active) { setDashboard(data); setError(false); } }).catch(() => { if (active) setError(true); }).finally(() => { if (active) setLoading(false); });
    return () => { active = false; };
  }, []);
    ...
    <main className="p-5 lg:p-7">
      ...
      {error ? <div className="rounded-xl border border-[#dbe1df] bg-white px-6 py-16 text-center text-sm text-[#657276]">A dashboard adatai nem tölthetők be.</div> : <>
        ...
        <div className="mt-5 grid gap-5 lg:grid-cols-2">
          {dashboard.followUps && <Card title="Saját utánkövetéseim"><div className="flex gap-5 border-b border-[#e6eae9] px-5 py-3 text-xs text-[#657276]"><span>Lejárt: <strong className="text-[#a34b3d]">{dashboard.followUps.overdue}</strong></span><span>Ma: <strong>{dashboard.followUps.today}</strong></span></div><div className="divide-y divide-[#e8eceb]">{dashboard.followUps.items.length ? dashboard.followUps.items.map((item) => <button key={item.id} type="button" onClick={() => navigate(`/follow-up/${item.id}`)} className="flex w-full cursor-pointer items-center justify-between gap-4 px-5 py-3.5 text-left hover:bg-[#fafcfc]"><span className="min-w-0"><strong className="block truncate text-xs">{item.lead.name}</strong><span className="mt-1 block text-[11px] text-[#84908e]">{followUpTypeLabels[item.type]}</span></span><time className="shrink-0 text-xs text-[#657276]">{formatFollowUpTime(item.dueAt)}</time></button>) : <p className="px-5 py-12 text-center text-xs text-[#7b8885]">Nincs nyitott saját utánkövetés.</p>}</div></Card>}
