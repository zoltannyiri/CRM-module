...
const controlClass = "h-9 cursor-pointer rounded-md border border-[#d6dddc] bg-white px-3 text-xs font-medium text-[#344247] outline-none hover:bg-[#f8f9f9] disabled:cursor-wait disabled:opacity-60";

export default function PipelinePage() {
  const { hasModule, hasPermission } = useAuth();
  const access = pipelineAccess(hasModule, hasPermission);
  const { showSuccess, showError } = useToast();
  const [definitions, setDefinitions] = useState({ pipelines: [], loaded: false, error: "" });
  const [selectedId, setSelectedId] = useState(null);
  const [query, setQuery] = useState("");
  const [assignedMemberId, setAssignedMemberId] = useState("");
  const [members, setMembers] = useState([]);
  const [reloadKey, setReloadKey] = useState(0);
  const [result, setResult] = useState({ board: null, resolvedKey: null, error: "" });
  ...
  useEffect(() => {
    ...
    let active = true;
    apiClient.get("/pipelines").then(({ data }) => { if (active) setDefinitions({ pipelines: data, loaded: true, error: "" }); })
      .catch((error) => { if (active) setDefinitions({ pipelines: [], loaded: true, error: error.response?.data?.message || "A Pipeline adatai nem tölthetők be." }); });
    return () => { active = false; };
  }, [access.view, reloadKey]);
  useEffect(() => {
    if (!access.view) return undefined;
    let active = true;
    apiClient.get("/members").then(({ data }) => { if (active) setMembers(data); }).catch(() => {});
    return () => { active = false; };
  }, [access.view]);
  useEffect(() => {
  ...
  return <div className="min-h-dvh bg-[#f3f5f6] text-[#253238]">
    ...
    <div className="flex flex-wrap items-center justify-between gap-3 border-b border-[#e3e8e6] bg-white px-5 py-3 lg:px-7">
      <div className="flex flex-wrap items-center gap-3"><h1 className="text-sm font-semibold">Pipeline</h1>{definitions.pipelines.length > 1 ? <select value={pipelineId || ""} onChange={(event) => setSelectedId(Number(event.target.value))} className={controlClass} aria-label="Pipeline kiválasztása">{definitions.pipelines.map((pipeline) => <option key={pipeline.id} value={pipeline.id}>{pipeline.name}{pipeline.isDefault ? " (alapértelmezett)" : ""}</option>)}</select> : <span className="text-xs text-[#71807c]">{current?.name}</span>}<select value={assignedMemberId} onChange={(event) => setAssignedMemberId(event.target.value)} className={controlClass} aria-label="Felelős szűrő"><option value="">Minden felelős</option>{members.map((member) => <option key={member.id} value={member.id}>{memberName(member)}</option>)}</select></div>
      <div className="flex gap-2">{pipelineId && access.edit && <button type="button" onClick={() => setForm({ mode: "edit", pipeline: current })} className={controlClass}>Beállítások</button>}{pipelineId && access.delete && <button type="button" disabled={deleting} onClick={removePipeline} className={`${controlClass} text-[#9d3c32]`}>Törlés</button>}{access.create && <button type="button" onClick={() => setForm({ mode: "create" })} className="h-9 cursor-pointer rounded-md border border-[#172a2e] bg-[#21343a] px-4 text-xs font-semibold text-white">Új Pipeline</button>}</div>
    </div>
    <div className="px-5 py-5 lg:px-7">
      {definitions.error ? <p role="alert" className="text-sm text-[#9a4335]">{definitions.error}</p> : !definitions.loaded || pipelineId && loading ? <div role="status" aria-label="Pipeline betöltése" className="grid min-h-64 place-items-center rounded-lg border border-[#dbe1df] bg-white"><i className="pi pi-spinner pi-spin text-2xl text-[#6fa675]" aria-hidden="true" /></div> : !pipelineId ? <p className="text-sm text-[#71807c]">Nincs megjeleníthető Pipeline.</p> : result.error ? <p role="alert" className="text-sm text-[#9a4335]">{result.error}</p> : result.board && <PipelineBoardComponent board={result.board} canEdit={access.edit} canViewFollowUps={hasModule("FOLLOW_UPS") && hasPermission("FOLLOW_UPS_VIEW")} moving={moving} onMove={move} />}
    </div>
    {form && <PipelineFormComponent {...form} onClose={() => setForm(null)} onSaved={(pipeline) => { setSelectedId(pipeline.id); setReloadKey((value) => value + 1); }} />}
  </div>;
}
