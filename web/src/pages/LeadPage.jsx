...
import {
  ...
  leadSourceLabels,
  memberName,
} from "../components/lead/leadDisplay.js";
...
export default function LeadPage() {
  ...
  useEffect(() => {
    ...
    apiClient
      ...
      .catch(() => {
        if (active) setMembersError("A felelősök nem tölthetők be.");
      })
      .finally(() => {
  ...
  const openCreate = () => {
    if (canCreate) setForm({ mode: "create", lead: null });
  };
  const openEdit = async (lead) => {
    if (!hasPermission("LEADS_EDIT")) return;
    try {
      const { data } = await apiClient.get(`/leads/${lead.id}`, {
        skipGlobalErrorToast: true,
      });
      setForm({ mode: "edit", lead: data });
    } catch (error) {
      showError(
        error.response?.data?.message || "Az érdeklődő nem tölthető be.",
      );
    }
  };
  const drawer = form && (
  ...
  const conversionDrawer = conversionLead && (
    <LeadConversionFormComponent
      lead={conversionLead}
    ...
    return (
      <div className="min-h-dvh bg-[#f3f5f6] text-[#253238]">
        ...
        <div className="flex items-center gap-3 border-b border-[#e3e8e6] bg-white px-5 py-3 lg:px-7">
          ...
          <h1 className="text-sm font-semibold">Érdeklődő adatlap</h1>
        </div>
        <div className="px-5 py-6 lg:px-7">
          <TabView className={tabsClass}>
            ...
            {hasModule("FOLLOW_UPS") && hasPermission("FOLLOW_UPS_VIEW") && (
              <TabPanel header="Utánkövetések">
                <RelatedFollowUpsComponent key={id} leadId={id} />
              </TabPanel>
            )}
          </TabView>
        </div>
    ...
    <div className="min-h-dvh bg-[#f3f5f6] text-[#253238]">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 border-y border-[#e3e8e6] bg-white px-5 py-3 lg:px-7">
        ...
        <div className="flex items-center gap-2">
          ...
          >
            Létrehozás{" "}
            <i
              className={`pi pi-chevron-down ml-2 text-[10px] ${sortDirection === "asc" ? "rotate-180" : ""}`}
              aria-hidden="true"
            />
          </button>
          {canCreate && (
      ...
      <div className="px-5 py-5">
        ...
        <LeadListComponent
          ...
          reloadKey={reloadKey}
          onView={(lead) => navigate(`/lead/${lead.id}`)}
          onEdit={openEdit}
