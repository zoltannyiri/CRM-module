...
export default function PartnerExportMenu({ query, typeFilter, sortDirection, buttonClassName }) {
  ...
  const downloadExport = async (format) => {
    ...
    try {
      ...
      const filename = disposition.match(/filename="?([^";]+)"?/i)?.[1]
        || `partnerek-${new Date().toISOString().slice(0, 10)}.${format}`;
  ...
  return (
    <div ref={containerRef} className="relative">
      <button type="button" onClick={() => setOpen((value) => !value)} aria-haspopup="menu" aria-expanded={open} className={buttonClassName}>
        <i className="pi pi-download text-[11px] text-[#748084]" aria-hidden="true" />
        Export
        <i className={`pi pi-chevron-down text-[9px] transition-transform ${open ? "rotate-180" : ""}`} aria-hidden="true" />
      </button>

      {open && (
        <div role="menu" className="absolute top-[calc(100%+6px)] right-0 z-30 w-64 overflow-hidden rounded-lg border border-[#d9e0de] bg-white p-1.5 shadow-[0_14px_35px_rgba(28,45,48,.14)]">
          <p className="px-3 pt-2 pb-1.5 text-[10px] font-semibold uppercase tracking-[.1em] text-[#8a9693]">Formátum kiválasztása</p>
          {formats.map((format) => (
            <button key={format.value} type="button" role="menuitem" disabled={Boolean(exporting)} onClick={() => downloadExport(format.value)} className="flex w-full cursor-pointer items-center gap-3 rounded-md border-0 bg-white px-3 py-2.5 text-left hover:bg-[#f2f5f4] disabled:cursor-wait disabled:opacity-55">
