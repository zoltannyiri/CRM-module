...
export default function PartnerDocumentsComponent({ partnerId }) {
  ...
  useEffect(() => {
    let active = true;
    apiClient
      .get("/documents", { params: { entityType: "PARTNER", entityId: partnerId } })
      .then(({ data }) => {
        if (active) {
          setDocuments(data);
          setError("");
  ...
  return (
    <section aria-labelledby="partner-documents-title">
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
        <div>
          <h2 id="partner-documents-title" className="text-base font-semibold text-[#29383d]">
            Dokumentumok
          </h2>
          <p className="mt-1 text-xs text-[#71807c]">A partnerhez tartozó dokumentumok listája.</p>
        </div>
        {canCreate && (
          <button
            type="button"
      ...
      >
        ...
        {loading ? (
          ...
          <div className="grid min-h-40 place-items-center px-6 text-center text-xs text-[#778286]">
            Ehhez a partnerhez még nincs dokumentum.
          </div>
        ) : (
