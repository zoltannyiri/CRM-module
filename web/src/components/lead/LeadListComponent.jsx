...
export default function LeadListComponent({
  ...
  const coreBody = (column, lead) => {
    if (column.key === "name") return <button type="button" onClick={() => onView?.(lead)} className="cursor-pointer border-0 bg-transparent p-0 text-left font-semibold text-[#263338] hover:underline">{lead.name}</button>;
    if (column.key === "status") return <span className={`inline-flex rounded-full border px-2.5 py-1 text-[10px] font-medium ${leadStatusClasses[lead.status]}`}>{leadStatusLabels[lead.status]}</span>;
    if (column.key === "source") return leadSourceLabels[lead.source];
    if (column.key === "assignedMember") return memberName(lead.assignedMember);
    if (column.key === "createdAt") return formatLeadDate(lead.createdAt);
